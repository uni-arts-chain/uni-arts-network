...
// --- crates ---
pub use codec::Codec;
use uniarts_primitives::{OpaqueBlock as Block, AccountId, Balance, Nonce, BlockNumber, AuraId, Hash, Header};
...
impl sc_client_api::UsageProvider<Block> for Client {
    fn usage_info(&self) -> sc_client_api::ClientInfo<Block> {
        match self {
            Self::Fuxi(client) => client.usage_info(),
...
impl sc_client_api::BlockBackend<Block> for Client {
    fn block_body(&self, id: &BlockId<Block>) -> sp_blockchain::Result<Option<Vec<<Block as BlockT>::Extrinsic>>> {
        match self {
            Self::Fuxi(client) => client.block_body(id),
    ...
    fn block(&self, id: &BlockId<Block>) -> sp_blockchain::Result<Option<SignedBlock<Block>>> {
        match self {
            Self::Fuxi(client) => client.block(id),
            Self::Pangu(client) => client.block(id),
        }
    }

    fn block_status(&self, id: &BlockId<Block>) -> sp_blockchain::Result<BlockStatus> {
        match self {
            Self::Fuxi(client) => client.block_status(id),
            Self::Pangu(client) => client.block_status(id),
        }
    }

    fn justification(&self, id: &BlockId<Block>) -> sp_blockchain::Result<Option<Justification>> {
        match self {
            Self::Fuxi(client) => client.justification(id),
            Self::Pangu(client) => client.justification(id),
        }
    }

    fn block_hash(&self, number: NumberFor<Block>) -> sp_blockchain::Result<Option<<Block as BlockT>::Hash>> {
        match self {
            Self::Fuxi(client) => client.block_hash(number),
    ...
    fn extrinsic(&self, hash: &<Block as BlockT>::Hash) -> sp_blockchain::Result<Option<<Block as BlockT>::Extrinsic>> {
        match self {
            Self::Fuxi(client) => client.extrinsic(hash),
            Self::Pangu(client) => client.extrinsic(hash),
        }
    }

    fn have_extrinsic(&self, hash: &<Block as BlockT>::Hash) -> sp_blockchain::Result<bool> {
        match self {
            Self::Fuxi(client) => client.have_extrinsic(hash),
            Self::Pangu(client) => client.have_extrinsic(hash),
        }
    }
}
...
impl sc_client_api::StorageProvider<Block, crate::service::FullBackend> for Client {
    fn storage(&self, id: &BlockId<Block>, key: &StorageKey) -> sp_blockchain::Result<Option<StorageData>> {
        match self {
            ...
            Self::Pangu(client) => client.storage(id, key),
        }
    }

    fn storage_keys(&self, id: &BlockId<Block>, key_prefix: &StorageKey) -> sp_blockchain::Result<Vec<StorageKey>> {
        match self {
            Self::Fuxi(client) => client.storage_keys(id, key_prefix),
            Self::Pangu(client) => client.storage_keys(id, key_prefix),
        }
    }
    ...
    fn storage_hash(
        ...
        match self {
            Self::Fuxi(client) => client.storage_hash(id, key),
            Self::Pangu(client) => client.storage_hash(id, key),
        }
    }
    ...
    fn storage_pairs(
        ...
        match self {
            Self::Fuxi(client) => client.storage_pairs(id, key_prefix),
            Self::Pangu(client) => client.storage_pairs(id, key_prefix),
        }
    }
    ...
    fn child_storage(
        ...
        match self {
            Self::Fuxi(client) => client.child_storage(id, child_info, key),
            Self::Pangu(client) => client.child_storage(id, child_info, key),
        }
    }
    ...
    fn child_storage_keys(
        ...
        match self {
            Self::Fuxi(client) => client.child_storage_keys(id, child_info, key_prefix),
            Self::Pangu(client) => client.child_storage_keys(id, child_info, key_prefix),
        }
    }
    ...
    fn child_storage_hash(
        ...
        match self {
            Self::Fuxi(client) => client.child_storage_hash(id, child_info, key),
            Self::Pangu(client) => client.child_storage_hash(id, child_info, key),
        }
    }
    ...
    fn max_key_changes_range(
        ...
        match self {
            Self::Fuxi(client) => client.max_key_changes_range(first, last),
            Self::Pangu(client) => client.max_key_changes_range(first, last),
        }
    }
    ...
    fn key_changes(
        ...
        match self {
            Self::Fuxi(client) => client.key_changes(first, last, storage_key, key),
            Self::Pangu(client) => client.key_changes(first, last, storage_key, key),
        }
    }
}
...
impl sp_blockchain::HeaderBackend<Block> for Client {
    fn header(&self, id: BlockId<Block>) -> sp_blockchain::Result<Option<Header>> {
        match self {
            Self::Fuxi(client) => client.header(&id),
            Self::Pangu(client) => client.header(&id),
        }
    }

    fn info(&self) -> sp_blockchain::Info<Block> {
        match self {
            Self::Fuxi(client) => client.info(),
            Self::Pangu(client) => client.info(),
        }
    }

    fn status(&self, id: BlockId<Block>) -> sp_blockchain::Result<sp_blockchain::BlockStatus> {
        match self {
            Self::Fuxi(client) => client.status(id),
            Self::Pangu(client) => client.status(id),
        }
    }

    fn number(&self, hash: Hash) -> sp_blockchain::Result<Option<BlockNumber>> {
        match self {
            Self::Fuxi(client) => client.number(hash),
            Self::Pangu(client) => client.number(hash),
        }
    }

    fn hash(&self, number: BlockNumber) -> sp_blockchain::Result<Option<Hash>> {
        match self {
            Self::Fuxi(client) => client.hash(number),
            Self::Pangu(client) => client.hash(number),
        }
    }
}